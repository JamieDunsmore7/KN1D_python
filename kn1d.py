# TODO: go back and sort out the bit where we can read in saved inputs
...
def KN1D(
    ...
    # Default collision flags (common KN1D_collisions)
    H2_H2_EL  = True  # include H2->H2 elastic self collisions
    H2_P_EL   = True  # include H2->H(+) elastic collisions
    H2_H_EL   = True  # include H2<->H elastic collisions
    H2_HP_CX  = True  # include H2->H2(+) charge exchange
    H_H_EL    = True  # include H->H elastic self collisions
    H_P_CX    = True  # include H->H(+) charge exchange
    H_P_EL    = True  # include H->H(+) elastic collisions
    Simple_CX = True  # simple CX source option (default)

    # Option: Read input parameters stored in file from previous run
    if ReadInput:
        input_file = f"{File}.KN1D_input"
        try:
            # TODO: restore the input parameters from the file
            saved = np.load(input_file, allow_pickle=True)
            # TODO: unpack saved variables back into x, Ti, Te, etc.
            # e.g x = saved['x']
        except Exception as e:
            print(prompt, 'Error reading file:', e)
            return None

    else:
        # Determine optimized vr, vx, x grid for Kinetic_H2 (molecules, M)
        nv    = 6
        Eneut = np.array([0.003, 0.01, 0.03, 0.1, 0.3, 1.0])
        fctr  = 0.3
        if GaugeH2 > 15.0:
            fctr *= 15.0 / GaugeH2
    ...
    # Section: Set up molecular flux BC from inputted neutral pressure
    ...
    # 2) Initialize fH and fH2 (possibly from previous run)
    if refine:
    ...
    # — CALL OUT: interpol →
    # IDL: NuLoss = interpol(Cs_LC, x, xH2)
    # Python analogue (you provided earlier?):
    # NuLoss = interpol(Cs_LC, x, xH2)
    NuLoss = interp1d(x, Cs_LC, kind='linear', bounds_error=False, fill_value=(Cs_LC[0], Cs_LC[-1]))(xH2)
    ...
    nCs_LC = n * Cs_LC
    SpH2_hat = interp1d(x, nCs_LC, kind='linear', bounds_error=False, fill_value=(nCs_LC[0], nCs_LC[-1]))(xH2)
    ...
    vxiM = interp1d(x, vxi, kind='linear', bounds_error=False, fill_value=(vxi[0], vxi[-1]))(xH2)
    vxiA = interp1d(x, vxi, kind='linear', bounds_error=False, fill_value=(vxi[0], vxi[-1]))(xH)
    ...
    if File:
        ...
        if old_data_present:
            ...
            if mesh_data['GaugeH2_s'] != GaugeH2: test += 1
            if mesh_data['mu_s'] != mu: test += 1
            ...
            if oldrun:
                ...
                H2_data = np.load(H2_file)
                if debrief:
                    print(f"{prompt} Reading output variables stored in {H_file}")
                H_data = np.load(H_file)

                # TODO: sort out the way these are restored from the files.
                fH_s   = fH
                fH2_s  = fH2
                nH2_s  = nH2
                SpH2_s = SpH2
            ...
            if not NewFile:
                print(f"{prompt} Cannot read old data: {mesh_file}, {H2_file}, {H_file}")
                error = 1
                return None

    elif NewFile:
        print(f"{prompt} ERROR: No file name specified")
        error = 1
        return None
    ...
    vbarM = 2 * vthM * np.sum(Vr2pidVrM * (fH2BC * (vxM * dVxM))) / nbarHMax #TODO: check this! it is a tensor product in the IDL. Same for the following lines
    vbarM_error = abs(vbarM - v0_bar) / max(vbarM, v0_bar)

    nvrm, nvxm = vrM.size, vxM.size
    vr2vx2 = np.empty((nvrm, nvxm))

    # Full Maxwell for TMax
    Max0     = Maxwell[:, :, 0]
    nbarMax  = np.sum(Vr2pidVrM * (Max0 * dVxM))
    UxMax    = vthM * np.sum(Vr2pidVrM * (Max0 * (vxM * dVxM))) / nbarMax

    for i in range(nvrm):
        vr2vx2[i, :] = vrM[i]**2 + (vxM - UxMax/vthM)**2

    TMax = (2*mu*mH) * vthM**2 * np.sum(Vr2pidVrM * ((vr2vx2 * Max0) * dVxM)) \
        / (3 * q * nbarMax)

    # Half‐Maxwell temperature THMax
    UxHMax = vthM * np.sum(Vr2pidVrM * (fH2BC * (vxM * dVxM))) / nbarHMax

    for i in range(nvrm):
        vr2vx2[i, :] = vrM[i]**2 + (vxM - UxHMax/vthM)**2

    THMax = (2*mu*mH) * vthM**2 * np.sum(Vr2pidVrM * ((vr2vx2 * fH2BC) * dVxM)) \
            / (3 * q * nbarHMax)

    if compute_errors and debrief:
        print(f"{prompt} VbarM_error:       {vbarM_error:.3e}")
        print(f"{prompt} TWall Maxwellian:   {TMax:.3e} eV")
        print(f"{prompt} TWall Half Maxwellian: {THMax:.3e} eV")
    ...
    while True: # this replaces the fH_fH2 iterate loop
        ...
        # Initialize iteration tracking
            ...
            # Save off previous nH2 to test convergence later
            nH2_prev = nH2.copy()

            # 10.1) Interpolate fH from the H mesh onto the H2 mesh ➞ fHM
            warn = 5e-3
            fHM = Interp_fVrVxX(
                fH, vrA, vxA, xH, TnormA,
                vrM, vxM, xH2, TnormM,
            ...
            kin2_result, kin2_seeds = kinetic_h2(
                vx=vxM,
                vr=vrM,
                x=xH2,
                Tnorm=TnormM,
            ...
            fH2           = kin2_result['fH2']
            nH2           = kin2_result['nH2']
            GammaxH2      = kin2_result['GammaxH2']
            VxH2          = kin2_result['VxH2']
            pH2           = kin2_result['pH2']
            TH2           = kin2_result['TH2']
            qxH2          = kin2_result['qxH2']
            qxH2_total    = kin2_result['qxH2_total']
            Sloss         = kin2_result['Sloss']
            QH2           = kin2_result['QH2']
            RxH2          = kin2_result['RxH2']
            QH2_total     = kin2_result['QH2_total']
            AlbedoH2      = kin2_result['AlbedoH2']
            WallH2        = kin2_result['WallH2']
            nHP           = kin2_result['nHP']
            THP           = kin2_result['THP']
            fSH           = kin2_result['fSH']
            SH            = kin2_result['SH']
            ...
            SHP           = kin2_result['SHP']
            NuE           = kin2_result['NuE']
            NuDis         = kin2_result['NuDis']
            ESH           = kin2_result['ESH']
            Eaxis         = kin2_result['Eaxis']
            # stress‐tensor & collision outputs
            piH2_xx       = kin2_result['piH2_xx']
            piH2_yy       = kin2_result['piH2_yy']
            piH2_zz       = kin2_result['piH2_zz']
            RxH2CX        = kin2_result['RxH2CX']
            EH2CX         = kin2_result['EH2CX']
            RxH_H2        = kin2_result['RxH_H2']
            RxP_H2        = kin2_result['RxP_H2']
            EH_H2         = kin2_result['EH_H2']
            EP_H2         = kin2_result['EP_H2']
            RxW_H2        = kin2_result['RxW_H2']
            EW_H2         = kin2_result['EW_H2']
            Epara_PerpH2_H2 = kin2_result['Epara_PerpH2_H2']
            # common Kinetic_H2_H_moments
            nHM = kin2_result['nH']
            VxHM = kin2_result['VxH']
            THM = kin2_result['TH']

            # and now update all of the “_s” seed variables for a possible restart:
            vx_s         = kin2_seeds['vx_s']
            vr_s         = kin2_seeds['vr_s']
            x_s          = kin2_seeds['x_s']
            Tnorm_s      = kin2_seeds['Tnorm_s']
            mu_s         = kin2_seeds['mu_s']
            Ti_s         = kin2_seeds['Ti_s']
            vxi_s        = kin2_seeds['vxi_s']
            Te_s         = kin2_seeds['Te_s']
            n_s          = kin2_seeds['n_s']
            fH2BC_s      = kin2_seeds['fH2BC_s']
            GammaxH2BC_s = kin2_seeds['GammaxH2BC_s']
            NuLoss_s     = kin2_seeds['NuLoss_s']
            PipeDia_s    = kin2_seeds['PipeDia_s']
            fH_s         = kin2_seeds['fH_s']
            SH2_s        = kin2_seeds['SH2_s']
            fH2_s        = kin2_seeds['fH2_s']
            nHP_s        = kin2_seeds['nHP_s']
            THP_s        = kin2_seeds['THP_s']
            Simple_CX_s  = kin2_seeds['Simple_CX_s']
            Sawada_s     = kin2_seeds['Sawada_s']
            H2_H2_EL_s   = kin2_seeds['H2_H2_EL_s']
            H2_P_EL_s    = kin2_seeds['H2_P_EL_s']
            H2_H_EL_s    = kin2_seeds['H2_H_EL_s']
            H2_HP_CX_s   = kin2_seeds['H2_HP_CX_s']
            ni_correct_s = kin2_seeds['ni_correct_s']
            ...
            fH2A = Interp_fVrVxX(
                fa=fH2, vra=vrM, vxa=vxM, xa=xH2, Tnorma=TnormM,
                vrb=vrA, vxb=vxA, xb=xH,   Tnormb=TnormA,
                debug=interp_debug, correct=True
            )

            fSHA = Interp_fVrVxX(
                fa=fSH, vra=vrM, vxa=vxM, xa=xH2, Tnorma=TnormM,
                vrb=vrA, vxb=vxA, xb=xH,   Tnormb=TnormA,
                debug=interp_debug, correct=True
            )
            ...
            # call your Python version of Kinetic_H
            h_results, h_seeds = kinetic_h(
            ...
            # unpack the common‐block outputs
            piH_xx        = h_results['piH_xx']
            piH_yy        = h_results['piH_yy']
            piH_zz        = h_results['piH_zz']
            RxHCX         = h_results['RxHCX']
            EHCX          = h_results['EHCX']
            RxH2_H        = h_results['RxH2_H']
            RxP_H         = h_results['RxP_H']
            EH2_H         = h_results['EH2_H']
            EP_H          = h_results['EP_H']
            RxW_H         = h_results['RxW_H']
            EW_H          = h_results['EW_H']
            Epara_PerpH_H = h_results['Epara_PerpH_H']
            SourceH       = h_results['SourceH']
            SRecomb       = h_results['SRecomb']

            # unpack the H2‐moments common block
            nH2A  = h_results['nH2']
            VxH2A = h_results['VxH2']
            TH2A  = h_results['TH2']
            ...
            if compute_errors:
                _RxH_H2 = interp_scalarx(RxH_H2, xH2, xH, warn=warn, debug=interp_debug)
                DRx     = _RxH_H2 + RxH2_H
                nDRx    = np.max(np.abs(DRx)) / np.max(np.abs([_RxH_H2, RxH2_H]))
                if debrief:
    ...
    # now we are in the fH_fH2_done section
    error = 0
    ...
    GammaHLim = interp1d(
        ...
        kind='linear',
        bounds_error=False,
        fill_value=(Gam[0], Gam[-1]) (xlimiter)
    )
    ...
    for k in range(nxH):
        fH_k = fH[:, :, k]
        # broadcast: Vr2pidVrA[:,None] * fH_k * (vxA* dVxA)[None,:]
    ...
    # 6) If the user asked to save to file, dump .npz files
    if File is not None and n_iter > 0:
        ...
        np.savez(f"{File}.KN1D_H2.npz",
            ...
            SP=SP, SHP=SHP,
            NuE=NuE, NuDis=NuDis, NuLoss=NuLoss,
            SpH2=SpH2,
            piH2_xx=piH2_xx, piH2_yy=piH2_yy, piH2_zz=piH2_zz,
            RxH2CX=RxH2CX, RxH_H2=RxH_H2, RxP_H2=RxP_H2,
            RxW_H2=RxW_H2, EH2CX=EH2CX, EH_H2=EH_H2,
            EP_H2=EP_H2, EW_H2=EW_H2,
        ...
        np.savez(f"{File}.KN1D_H.npz",
            ...
            piH_xx=piH_xx, piH_yy=piH_yy, piH_zz=piH_zz,
            RxHCX=RxHCX, RxH2_H=RxH2_H, RxP_H=RxP_H,
            RxW_H=RxW_H, EHCX=EHCX,
            EH2_H=EH2_H, EP_H=EP_H, EW_H=EW_H,
            Epara_PerpH_H=Epara_PerpH_H,
            SourceH=SourceH, SRecomb=SRecomb,
        ...
        if plot:
            # precompute all our “mid‐point” indices in one shot
            ...
            ax.plot(  xH,  GammaHLim*fscale,  color='C3')
            ax.text(xH[midH[3]], 1.1*GammaHLim[midH[3]]*fscale, f"2×{_HH}+{_H}", color='C3')
            ...
            # ————————————————————————————————————————————————————————————————
            # (if you haven’t already computed them)
            ...
            ydata = np.hstack([SH, SP, SHP, SpH2, SideWallH,
                            NuLoss*nHP, NuDis*nHP, Sion, SRecomb])
            ax.set_yscale('log')
            ...
            ax.plot(xH, SRecomb);      ax.text(xH[midH[1]],    1.1*SRecomb[midH[1]],   '+'+_H+'(Rec)')
            kn1d_limiter_include(ax, xlimiter, xsep)
            plt.tight_layout()
            plt.show()
            ...
            fig, ax = plt.subplots()
            mid, midH, midH2 = kn1d_include(ax, x, xH, xH2, GaugeH2, File, HH_label=_HH)
            ydata = np.hstack([RxH2CX, RxH_H2, RxP_H2, RxW_H2, RxHCX, RxH2_H, RxP_H, RxW_H])
            ax.set_ylim((ydata.min(), ydata.max()))
            ax.set_title("KN1D: x-Momentum Transfer Rates")
            ax.set_xlabel("x (m)"); ax.set_ylabel("N m^-3")
            ax.plot(xH2, RxH2CX);    ax.text(xH2[midH2[3]], RxH2CX[midH2[3]],   _p+'→'+_HH)
            ax.plot(xH2, RxH_H2);    ax.text(xH2[midH2[1]], 1.2*RxH_H2[midH2[1]],  _H+'→'+_HH)
            ax.plot(xH2, RxP_H2);    ax.text(xH2[midH2[2]], RxP_H2[midH2[2]],    _p+'→'+_HH)
            ax.plot(xH2,-RxW_H2);    ax.text(xH2[midH2[0]], -1.2*RxW_H2[midH2[0]], _HH+'→Side Wall')
            ax.plot(xH,  RxHCX);     ax.text(xH[midH[4]],    RxHCX[midH[4]],      _p+'→'+_H+'(CX)')
            ax.plot(xH,  RxH2_H);    ax.text(xH[midH[1]], 1.2*RxH2_H[midH[1]], _HH+'→'+_H)
            ax.plot(xH,  RxP_H);     ax.text(xH[midH[5]],    RxP_H[midH[5]],      _p+'→'+_H+'(EL)')
            ax.plot(xH, -RxW_H);     ax.text(xH[midH[0]], -1.2*RxW_H[midH[0]],   _H+'→Side Wall')
            kn1d_limiter_include(ax, xlimiter, xsep)
            plt.tight_layout()
            plt.show()
            ...
            mid, midH, midH2 = kn1d_include(ax, x, xH, xH2, GaugeH2, File, HH_label=_HH)
            f = 1e-6
            ydata = np.hstack([EH2CX, EH_H2, EP_H2, EW_H2, EHCX, EH2_H, EP_H, EW_H]) * f
            ax.set_ylim((ydata.min(), ydata.max()))
            ax.set_title("KN1D: Energy Transfer Rates")
            ax.set_xlabel("x (m)"); ax.set_ylabel("MW m^-3")
            ax.plot(xH2, EH2CX*f);  ax.text(xH2[midH2[2]], EH2CX[midH2[2]]*f, _p+'→'+_HH)
            ax.plot(xH2, EH_H2*f);  ax.text(xH2[midH2[1]], EH_H2[midH2[1]]*f, _H+'→'+_HH)
            ax.plot(xH2, EP_H2*f);  ax.text(xH2[midH2[3]], EP_H2[midH2[3]]*f, _p+'→'+_HH)
            ax.plot(xH2,-EW_H2*f);  ax.text(xH2[midH2[0]],-EW_H2[midH2[0]]*f, _HH+'→Side Wall')
            ax.plot(xH,  EHCX*f);   ax.text(xH[midH[5]],    EHCX[midH[5]]*f,  _p+'→'+_H+'(CX)')
            ax.plot(xH,  EH2_H*f);  ax.text(xH[midH[4]],    EH2_H[midH[4]]*f, _HH+'→'+_H)
            ax.plot(xH,  EP_H*f);   ax.text(xH[midH[4]],    EP_H[midH[4]]*f,  _p+'→'+_H+'(EL)')
            ax.plot(xH,-EW_H*f);    ax.text(xH[midH[0]],   -EW_H[midH[0]]*f,  _H+'→Side Wall')
            kn1d_limiter_include(ax, xlimiter, xsep)
            plt.tight_layout()
            plt.show()
            ...
            fig, ax = plt.subplots()
            mid, midH, midH2 = kn1d_include(ax, x, xH, xH2, GaugeH2, File, HH_label=_HH)
            ydata = np.hstack([Epara_PerpH2_H2, Epara_PerpH_H])
            ax.set_ylim((ydata.min(), ydata.max()))
            ax.set_title("KN1D: T⊥→T∥ Isotropization Rates")
            ax.set_xlabel("x (m)"); ax.set_ylabel("W m^-3")
            ax.plot(xH2, Epara_PerpH2_H2); ax.text(xH2[midH2[2]], Epara_PerpH2_H2[midH2[2]], _HH+'↔'+_HH+'(EL)')
            ax.plot(xH,  Epara_PerpH_H);  ax.text(xH[midH[3]],  Epara_PerpH_H[midH[3]],  _H+'↔'+_H+'(EL)')
            kn1d_limiter_include(ax, xlimiter, xsep)
            plt.tight_layout()
            plt.show()
            ...
            ax.plot(xH, 100*Balmer*f); ax.text(xH[midH[4]], 100*Balmer[midH[4]]*f, 'Balmer×100')
